...
- `/api/ops/security-evidence/gate` (GET, viewer+, supports `max_age_seconds` + `min_retention_days` + `required_checks` + `strict_required_checks`; includes `gate_reason_code` + `gate_failed_count` + `strict_required_checks` + `defaulted_checks` + `requested_checks` + `ignored_checks` + `duplicate_checks` + `evaluated_checks` + `failed_checks` for automation)
...
- `503` gate failure response is schema-based (`OpsSecurityEvidenceGateErrorResponse`) and includes `gate_reason_code` + `gate_failed_count` + `strict_required_checks` + `defaulted_checks` + `requested_checks` + `ignored_checks` + `duplicate_checks` + `evaluated_checks` + `failed_checks`
...
- Contracts registry version for gate family is now `1.8`
