...
- `/api/ops/security-evidence/gate` (GET, viewer+, supports `max_age_seconds` + `min_retention_days` + `required_checks` + `strict_required_checks`; includes `strict_required_checks` + `requested_checks` + `ignored_checks` + `evaluated_checks` + `failed_checks` for automation)
...
- `503` gate failure response is schema-based (`OpsSecurityEvidenceGateErrorResponse`) and includes `strict_required_checks` + `requested_checks` + `ignored_checks` + `evaluated_checks` + `failed_checks`
...
- Contracts registry version for gate family is now `1.4`
